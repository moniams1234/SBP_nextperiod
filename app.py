...
with tabs[0]:
    ...
    with c1:
        program_label = st.selectbox("Program type", list(PROGRAM_TYPES.keys()))
        instrument_type = st.selectbox("Instrument type",
                                       ["Stock option", "Warrant", "RSU / Performance share"])
        grant_date = st.date_input("Grant date", value=dt.date.today())
    with c2:
        reporting_date = st.date_input("Reporting date", value=dt.date.today())
        program_start = st.date_input("Program start date", value=dt.date.today())
        vesting_period = st.number_input("Vesting period (years)", min_value=0.0, value=3.0, step=0.5)
    with c3:
        expected_life = st.number_input("Total expected life (years)", min_value=0.0, value=5.0, step=0.5)
        n_employees = st.number_input("Number of employees", min_value=0, value=50, step=1)
        per_employee = st.number_input("Options / warrants per employee", min_value=0, value=1000, step=100)

    c4, c5, c6 = st.columns(3)
    with c4:
        total_granted = st.number_input("Total instruments granted",
                                        min_value=0, value=int(n_employees * per_employee), step=100)
    with c5:
    ...
    with c6:
        expected_vest_pct = st.number_input("Expected % vesting (override)",
                                            ...
                                            value=100.0 - forfeiture_rate_pct, step=0.5)
...
with tabs[1]:
    ...
    c1, c2, c3 = st.columns(3)
    with c1:
        S0 = st.number_input("Current share price S₀", min_value=0.0001, value=100.0, step=1.0, format="%.4f")
    ...
    with c3:
        T = st.number_input("Time to maturity T (years)", min_value=0.0001,
                            value=float(ss.program.get("expected_life_years", 5.0)), step=0.25)
        opt_type = st.selectbox("Option type used for IFRS 2", ["Call", "Put"])
    ...
    sigma_pct = st.number_input("Volatility σ (%) — auto from Tab 3 if computed",
                                min_value=0.01, value=sigma_default * 100.0, step=1.0)

    if st.button("Compute Black-Scholes"):
        try:
...
with tabs[2]:
    ...
    method = st.radio("Method", ["Manual input", "Upload historical prices"], horizontal=True)

    if method == "Manual input":
        man = st.number_input("Annual volatility (%)", min_value=0.0, value=30.0, step=1.0)
        ...
        if up is not None:
            try:
                ...
                c1, c2 = st.columns(2)
                with c1:
                    by_name = st.selectbox("Price column (by name)", cols)
                with c2:
                    by_idx = st.number_input("…or column number (1-based, 0 = ignore)",
                                             min_value=0, max_value=len(cols), value=0)
                freq = st.selectbox("Frequency", ["daily", "weekly", "monthly"])
                if st.button("Compute volatility"):
...
with tabs[3]:
    ...
    with c3:
        vesting_yrs = st.number_input("Vesting period (years)", min_value=0.0,
                                      value=float(p.get("vesting_period_years", 3.0)), step=0.5)
        service_ok = st.selectbox("Service condition fulfilled?", ["Yes", "No"])
        perf_ok = st.selectbox("Non-market performance condition fulfilled?", ["Yes", "No"])

    expected_vest_pct = st.slider("Expected % of instruments vesting", 0.0, 100.0,
                                  float(100.0 - forfeit_pct), step=0.5)
...
with tabs[4]:
    ...
    c1, c2, c3 = st.columns(3)
    with c1:
        months = st.number_input("Months active in first reporting year", min_value=0, max_value=12, value=12)
    with c2:
        fv_default = float(b.get("selected_fv", 0.0)) or 0.0
        fv_per = st.number_input("Grant-date FV per option", min_value=0.0,
                                 value=fv_default, step=0.01, format="%.4f")
    with c3:
    ...
    if program_type == "choice":
        ...
        with c1:
            has_liability = st.checkbox("Present obligation to settle in cash?", value=True)
        with c2:
            liability_val = st.number_input("Cash-settled liability value", min_value=0.0, value=0.0)
        with c3:
            cash_alt = st.number_input("Cash alternative value", min_value=0.0, value=0.0)
        with c4:
            equity_alt = st.number_input("Equity alternative value", min_value=0.0, value=0.0)

    if st.button("Compute first-year accounting"):
        try:
...
with tabs[5]:
    ...
    subtype_label = st.selectbox("Award type for subsequent period",
                                 ...
                                 index=list(PROGRAM_TYPES.values()).index(
                                     ss.program.get("program_type", "equity")))
    ...
    if subtype == "equity":
        ...
        with c1:
            fv = st.number_input("Grant-date FV per option",
                                 value=float(ss.bs.get("selected_fv", 0.0)) or 0.0, format="%.4f")
            granted = st.number_input("Original instruments granted", min_value=0,
                                      value=int(p.get("total_granted", 0)))
        with c2:
            ...
            vest_yrs = st.number_input("Vesting period (years)", min_value=0.01,
                                       value=float(p.get("vesting_period_years", 3.0)))
        with c3:
        ...
        if st.button("Compute equity subsequent"):
        ...
        with c1:
            S0 = st.number_input("Current share price S₀", min_value=0.0001,
                                 value=float(ss.bs.get("S0", 100.0)))
            K = st.number_input("Current strike K", min_value=0.0001,
                                value=float(ss.bs.get("K", 100.0)))
            sigma_pct = st.number_input("Current volatility σ (%)", min_value=0.01,
                                        value=float(ss.bs.get("sigma", 0.3))*100, step=1.0)
        with c2:
            r_pct = st.number_input("Risk-free r (%)", value=float(ss.bs.get("r", 0.05))*100, step=0.25)
            q_pct = st.number_input("Dividend yield q (%)", value=float(ss.bs.get("q", 0.0))*100, step=0.25)
            T_rem = st.number_input("Remaining expected life (years)", min_value=0.01,
                                    value=float(ss.bs.get("T", 3.0)))
        with c3:
            outstanding = st.number_input("Instruments outstanding", min_value=0,
                                          value=int(p.get("total_granted", 0)))
            upd_forf = st.number_input("Updated forfeiture rate (%)", min_value=0.0, max_value=100.0,
                                       value=float(p.get("forfeiture_rate", 0.1))*100, step=0.5)
            opening_liab = st.number_input("Opening liability (prior period)", min_value=0.0, value=0.0)
        c4, c5 = st.columns(2)
        with c4:
            vest_progress = st.slider("Vesting service progress", 0.0, 1.0, 0.5, 0.05)
        with c5:
            cash_paid = st.number_input("Cash paid / settled during period", min_value=0.0, value=0.0)

        if st.button("Compute cash subsequent"):
            try:
        ...
        c1, c2, c3 = st.columns(3)
        with c1:
            has_obl = st.checkbox("Present obligation to settle in cash?", value=True)
            opening_liab = st.number_input("Opening liability", min_value=0.0, value=0.0)
        with c2:
            cash_fv = st.number_input("Updated cash alternative FV / instr.", min_value=0.0, value=0.0)
            equity_fv = st.number_input("Updated equity alternative FV / instr.", min_value=0.0, value=0.0)
        with c3:
            upd_vested = st.number_input("Updated expected vested instruments", min_value=0.0,
                                         value=float(p.get("expected_vested", 0.0)))
            vest_progress = st.slider("Vesting service progress", 0.0, 1.0, 0.5, 0.05)
        cash_paid = st.number_input("Cash paid during period", min_value=0.0, value=0.0)
        st.warning("Classification of compound instruments depends on the exact IFRS 2 terms — confirm with auditors.")

        if st.button("Compute choice subsequent"):
